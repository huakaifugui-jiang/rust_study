/*
 * @Author: wlj
 * @Date: 2022-12-27 09:22:45
 * @LastEditors: wlj
 * @LastEditTime: 2022-12-27 17:36:25
 * @Description: async编程入门
 * @see:https://course.rs/async-rust/async/getting-started.html rust圣经
 */
/// # Async 编程简介
/// 简单来说，异步编程是一个并发编程模型，目前主流语言基本都支持了，当然，支持的方式有所不同。异步编程允许我们同时并发运行大量的任务，
/// 却仅仅需要几个甚至一个 OS 线程或 CPU 核心，现代化的异步编程在使用体验上跟同步编程也几无区别
/// 例如 Go 语言的 go 关键字，也包括我们后面将介绍的 async/await 语法，该语法是 JavaScript 和 Rust 的核心特性之一。
/// # async 简介
/// async 是Rust选择的异步编程模型，下面我们来介绍下它的优缺点，以及何时适合使用。
///
/// ## async vs 其它并发模型
/// 由于并发编程在现代社会非常重要，因此每个主流语言都对自己的并发模型进行过权衡取舍和精心设计，Rust 语言也不例外。下面的列表可以帮助大家理解不同并发模型的取舍:
/// - OS 线程, 它最简单，也无需改变任何编程模型(业务/代码逻辑)，因此非常适合作为语言的原生并发模型，我们在多线程章节也提到过，Rust 就选择了原生支持线程级的并发编程。但是，这种模型也有缺点，
...
/// 这种模型性能相当的好，但最大的问题就是存在回调地狱的风险：非线性的控制流和结果处理导致了数据流向和错误传播变得难以掌控，还会导致代码可维护性和可读性的大幅降低，大名鼎鼎的 JavaScript 曾经就存在回调地狱。
/// - 协程(Coroutines) 可能是目前最火的并发模型,Go 语言的协程设计就非常优秀，这也是 Go 语言能够迅速火遍全球的杀手锏之一。协程跟线程类似，无需改变编程模型，同时，它也跟 async 类似，
/// 可以支持大量的任务并发运行。但协程抽象层次过高，导致用户无法接触到底层的细节，这对于系统编程语言和自定义异步运行时是难以接受的
/// - actor 模型是 erlang 的杀手锏之一，它将所有并发计算分割成一个一个单元，这些单元被称为 actor , 单元之间通过消息传递的方式进行通信和数据传递，跟分布式系统的设计理念非常相像。
...
/// - Future 在 Rust 中是惰性的，只有在被轮询(poll)时才会运行， 因此丢弃一个future会阻止它未来再被运行，你可以将Future理解为一个在未来某个时间点被调度执行的任务。
/// - Async在Rust中使用开销是零，意味着只有你能看到的代码（自己的代码）才有性能损耗，你看不到的（async内部实现）都没有性能损耗，例如，你可以无需分配任何堆内存
/// 、也无需任何动态分发来使用async，这对于热点路径的性能有非常大的好处，正是得益与此，Rust的异步编程性能才会这么高。
/// - Rust 没有内置异步调用所需的运行时，但是无需担心，Rust社区生态中已经提供了非常优异的运行时实现，例如大明星 tokio
...
///
/// 而高并发更适合 IO 密集型任务，例如 web 服务器、数据库连接等等网络服务，因为这些任务绝大部分时间都处于等待状态，如果使用多线程，那线程大量时间会处于无所事事的状态
/// 再加上线程上下文切换的高昂代价，让多线程做 IO 密集任务变成了一件非常奢侈的事。而使用async，既可以有效的降低 CPU 和内存的负担，又可以让大量的任务并发的运行，
...
///
/// 总之，async编程并没有比多线程更好，最终还是根据你的使用场景作出合适的选择，如果无需高并发，或者也不在意线程切换带来的性能损耗，那么多线程使用起来会简单、方便的多！
///
/// 若大家使用 tokio，那 CPU 密集的任务尤其需要用线程的方式去处理，例如使用 spawn_blocking 创建一个阻塞的线程去完成相应 CPU 密集任务。
///
/// 至于具体的原因，不仅是上文说到的那些，还有一个是：tokio 是协作式地调度器，如果某个 CPU 密集的异步任务是通过 tokio 创建的，那理论上来说，该异步任务需要跟其它的异步任务交错执行
...
/// - 无所谓时，统一选多线程
/// # Async Rust 当前的进展
/// 简而言之，ust 语言的 async 目前还没有达到多线程的成熟度，其中一部分内容还在不断进化中，当然，这并不影响我们在生产级项目中使用，因为社区中还有 tokio 这种大杀器。
///
...
/// 因此要完整的使用 async 异步编程，你需要依赖以下特性和外部库:
...
/// 还有，你在同步( synchronous )代码中使用的一些语言特性在 async 中可能将无法再使用，而且 Rust 也不允许你在特征中声明 async 函数(可以通过三方库实现)，
/// 总之，你会遇到一些在同步代码中不会遇到的奇奇怪怪、形形色色的问题，不过不用担心，本章会专门用一个章节罗列这些问题，并给出相应的解决方案。
/// ## 编译和错误
...
/// 好在一旦你深入学习并掌握了本章的内容和 async 原理，可以有效的降低遇到这些错误的概率
///
/// ## 兼容性考虑
...
///
/// 但是世事皆有例外。目前主流的 async 运行时几乎都使用了多线程实现，相比单线程虽然增加了并发表现，但是对于执行性能会有所损失
/// 因为多线程实现会有同步和切换上的性能开销，若你需要极致的顺序执行性能，那么 async 目前并不是一个好的选择。
///
/// 同样的，对于延迟敏感的任务来说，任务的执行次序需要能被严格掌控，而不是交由运行时去自动调度，后者会导致不可预知的延迟，例如一个 web 服务器总是有 1% 的请求，它们的延迟会远高于其它请求，
/// 因为调度过于繁忙导致了部分任务被延迟调度，最终导致了较高的延时。正因为此，这些延迟敏感的任务非常依赖于运行时或操作系统提供调度次序上的支持。
///
...
fn main() {
    ...
    //     let thread_two = thread::spawn(|| download("https://course.rs"));
    ...
    //如果是在一个小项目中简单的去下载文件，这么写就没有任何问题，但是一旦下载文件的并发请求躲起来，那一个下载任务占用一个线程的模式就太重了，会很容易成为程序的瓶颈。
    ...
    // 通过 async 标记的语法块会被转换成实现了Future特征的状态机。与同步调用阻塞当前线程不同，当Future执行并遇到阻塞时，它会让出当前线程的控制权
    ...
    // do_something(); //会有警告：unused implementer of `Future` that must be used
    //告诉我们 Future 未被使用，那么到底该如何使用？答案是使用一个执行器( executor ):
    use futures::executor::block_on;
    // let future = hello_world();
    //`block_on`会阻塞当前线程直到指定的`Future`执行完成，这种阻塞当前线程以等待任务完成的方式较为简单、粗暴
    //好在其他运行时的执行器 executor 会提供更加复杂的行为，例如将多个`future`调度到同一个线程上执行。
    // block_on(future); //执行`Future`并等待其运行完成，此时"hello, world!"会被打印输出
    // println!("main thread");

    //在上述代码的main函数中，我们使用block_on这个执行器等待Future的完成，让代码看上去非常像是同步代码，
    //但是如果你要在一个async fn函数中去调用另一个async fn并等待其完成后再执行后续的代码，该如何做？
    println!("before async");
    async fn hello_cat() {
        println!("hello, kitty!");
    }
    async fn hello_world() {
        hello_cat().await; //译器友善的给出了提示：futures do nothing unless you `.await` or poll them，两种解决方法：使用.await语法或者对Future进行轮询(poll)。
                           //在async fn函数中使用.await可以等待另一个异步调用的完成。但是与block_on不同，.await并不会阻塞当前的线程，
                           //而是异步的等待Future A的完成，在等待的过程中，该线程还可以继续执行其它的Future B，最终实现了并发处理的效果。
        println!("hello, world!");
    }
    let future = hello_world();
    block_on(future); //只会打印 hello，world

    println!("after async");

    //一个例子
    //考虑到一个载歌载舞的例子，如果不用.await，我们可能会有如下实现：
    struct Song {
        author: String,
        name: String,
    }
    ...
    block_on(async_main());
    //上面代码中，学歌和唱歌具有明显的先后顺序，但是这两者都可以跟跳舞一同存在，也就是你可以在跳舞的时候学歌，也可以在跳舞的时候唱歌。
    //如果上面代码不使用.await，而是使用block_on(learn_song())， 那在学歌时，当前线程就会阻塞，不再可以做其它任何事，包括跳舞。
    //
}
